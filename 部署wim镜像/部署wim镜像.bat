...
pause
::自定义分区
diskpart /s tools\format.txt

::部署镜像
dism /apply-image /imagefile:wims\install.wim /index:1 /applydir:F:\

::设置引导
bcdboot F:\windows /s F: /l zh-cn /f ALL

::复制常用工具，安装包到系统盘
xcopy tools\ F:\tools\ /E /F

::安装主板,显卡,网卡,打印机等驱动，可以在安装好的主机上，输入dism /online /Export-Driver /Destination:C:\destpath,把所需的驱动都导出。放在drivers文件夹。
dism /image:F:\ /add-driver /driver:drivers\ /recurse /forceunsigned

::安装补丁
dism /image:F:\ /add-package /packagePath:hotfixs\

::设置自动化,可以去http://www.windowsafg.com这里，生成一个适用于自己的应答文件。
dism /image:F: /apply-unattend:unattend\unattend-8.1.xml
